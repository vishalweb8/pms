@extends($theme)
@if(url()->previous() == route('leave-all-employee'))
<?php $breadcrumb = Breadcrumbs::render('all-leave-view'); ?>
@else
<?php $breadcrumb = Breadcrumbs::render('team-leave-view'); ?>
@endif
@section('breadcrumbs')
{{ $breadcrumb }}
@endsection
            ...
            <div class="btn-outer">
                ...
                <span class="leave-status-div">
                    <a href="javascript:void(0)"
                        ...
                        data-action="approve_leave" data-id="{{ $leaveView->id }}"
                    ...
                    <a href="javascript:void(0)" class="btn btn-danger waves-effect waves-light cancel-btn reject_leave"
                        data-action="reject_leave" data-id="{{ $leaveView->id }}"
                ...
                <a class="btn btn-secondary waves-effect waves-light cancel-btn"
                    href="{{  (url()->previous() == route('leave-all-employee') ? route('leave-all-employee') : route('leave-team')) }}">Back</a>
            </div>
        </div>
    </div>
</div>
                            ...
                            <div class="media-img">
                                ...
                                <div class="align-self-center media-body mt-1">
                                    <div class="text-muted">
                                        ...
                                        <p class="mb-2"> {{($leaveUser->officialUser) ? (($leaveUser->officialUser->userTeam) ?
                                                $leaveUser->officialUser->userTeam->name : '') : ''}}
                                        </p>
                                    </div>
                                </div>
                            </div>
