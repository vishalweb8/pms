<div class="d-flex justify-content-between ">
    <div class="mb-3 font-size-18 fw-normal">
        ...
        @if ($leaveView->status == "pending")
        <span class='badge badge-light-warning'>Pending</span>
        @endif
        @if ($leaveView->status == "approved")
        <span class='badge badge-light-success'>Approved</span>
        @endif
        @if ($leaveView->status == "rejected")
        <span class='badge badge-light-danger'>Rejected</span>
        @endif
        @if ($leaveView->status == "cancelled")
        <span class='badge badge-light-info'>Cancelled</span>
        @endif
    </div>
    @if($leaveView->created_by != NULL)
    <div class="mb-6 font-size-18 fw-normal">
        <span class="mb-3 font-size-14 fw-normal">Requested By</span>
        <span class='badge badge-light-default'>{{($leaveView->created_by != NULL) ? $leaveView->userCreated->full_name : ''}}</span>
    </div>
    @endif
</div>
@if(in_array(request()->route()->getName(),['leaves-comments-view','wfh-comments-view']))
        ...
        <div class="card-body p-2">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <span class="mb-0 reason-box">{{ $leaveView->reason }}</span>
                </div>
            </div>
        </div>
    </div>
...
<ul class="status-timeline">
    <li class="mb-2">
        @forelse ( $leaveComments as $comments )
        <div class="status-update-outer">
            ...
            <div class="status-detail">
                ...
                <div class="user">
                    @if($comments->reviewUser->profile_image &&
                    file_exists(public_path('storage/upload/user/images/'.$comments->reviewUser->profile_image)))
                    <img class="w-100" alt="img"
                        src="{{asset('storage/upload/user/images')}}/{{$comments->reviewUser->profile_image}}">
                    @else
                    <div class="avatar-xs">
        ...
        @empty
        @if($leaveView->status != "cancelled")
        <div class="comment-not-found d-flex justify-content-center align-items-center h-100 flex-column">
