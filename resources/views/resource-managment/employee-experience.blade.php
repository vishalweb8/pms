        ...
        <div class="card emp-list-section ">
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <input type="hidden" value="employeeExpPage" id="checkPage">
                    <table class="table table-centered  mb-0" id="teamFilterDiv">
                            ...
                            <tr>
                                <th class="p-1 bg-white select-project-team">
                                    @if(in_array(\Auth::user()->userRole->code, ['ADMIN', 'PM']))
                                        {!! Form::select('team_id', $teams, null, ['class' => 'form-control form-select2 select2', 'id'=> "team_emp_exp"]) !!}
                                    @else
                                        <select class="form-control select2" name="team_id" id="team_emp_exp" disabled>
                        ...
                        <tbody id="allEmployeesExp"></tbody>
                    </table>
                </div>
                 <!-- Data Loader -->
...
<script type="text/javascript">
    var ENDPOINT = "{{ url('/') }}";
    var page = 1;
    var siteUrl = ENDPOINT + "/admin-employee-experience?page=" + page;
    if(fixed_team) {
        siteUrl += `&team_id=${fixed_team}`;
    }
    listAjaxCall(siteUrl, "allEmployeesExp");
</script>
@endpush
