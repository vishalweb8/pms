    ...
    <td>
        <table class="section" border="0" cellpadding="0" cellspacing="0" align="center" width="100%" style="background:#ffffff;">
            ...
            <!--leave--request--content-->
                ...
                <tr>
                    <td>
                       <h2 style="text-align: center; color: #374A5E;">Leave Request</h2>
                    </td>
                 </tr>
                <tr>
                    <td>
                        ...
                        <p style="padding: 0px 20px; font-size: 14px; color: #707070; line-height: 25px; padding-bottom: 10px;">
                            I ({{ $applied_person->full_name ? ucwords($applied_person->full_name) : '-' }} - {{ $applied_person->email ? $applied_person->email : '-' }}) would like to request {{ ($leaveDetails->duration) ? $leaveDetails->duration . (($leaveDetails->duration == 1) ? ' leave day' : ' leave days') : '-' }} starting from {{ $leaveDetails->start_date ? $leaveDetails->start_date : '-' }} due to {{ $leaveDetails->reason ? $leaveDetails->reason : '-' }}.</p>
                        ...
                        @if($leaveDetails->isadhoc_leave == 0)
                         <p style="padding: 0px 20px; font-size: 14px; color: #707070; line-height: 25px; padding-bottom: 10px;">
                           You can Approve or Reject a Leave request by clicking on the below link.</p>
                           <a style="padding: 0px 20px; font-size: 14px; line-height: 25px; padding-bottom: 10px;" href="{{ (!empty($action_url))? $action_url : route('leave-team-view',$leaveDetails->id) }}">Click here for approve or reject leave</a>
                        @endif
                    </td>
                </tr>
            </tbody>
