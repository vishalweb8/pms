    ...
    <td>
        <table class="section" border="0" cellpadding="0" cellspacing="0" align="center" width="100%" style="background:#ffffff;">
            <tbody>
                ...
                <tr>
                    <td>
                        <h2 style="text-align: center; color: #374A5E;">Leave Compensation Request Cancelled</h2>
                    </td>
                </tr>
                <tr>
                    <td> <h4 style="padding: 0px 20px; color: #374A5E;">Hello {{ $leaveDetails->userLeave->full_name ? ucwords($leaveDetails->userLeave->full_name) : '-' }},</h4>
                           <p style="padding: 0px 20px; font-size: 14px; color: #707070; line-height: 25px; padding-bottom: 10px;">
                            Your leave compensation request for {{ $leaveDetails->start_date ? (\Carbon\Carbon::parse($leaveDetails->start_date)->isoFormat('DD MMM YYYY')) : '-' }} to {{ $leaveDetails->end_date ? (\Carbon\Carbon::parse($leaveDetails->end_date)->isoFormat('DD MMM YYYY')) : '-' }} is cancelled by {{ $leave_comment->reviewUser->full_name ? ucwords($leave_comment->reviewUser->full_name) : '-' }}. </p>
                            <p style="padding: 0px 20px; font-size: 14px; color: #707070; line-height: 25px; padding-bottom: 10px;">
