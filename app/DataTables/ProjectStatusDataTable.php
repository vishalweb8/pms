...
class ProjectStatusDataTable extends DataTable
{
    ...
    public function applyFilter($query)
    {
        if(request()->order[0]['column'] != null){
            ...
            if(Schema::hasColumn('project_status',$column_name)){
    ...
    {
        $options = collect([
            ['btn_file_name' => 'edit', 'permission' => 'project-status.edit', 'link' => route('project-status.edit', $row->id),],
            // ['btn_file_name' => 'delete', 'permission' => 'project-status.destroy', 'link' => route('project-status.destroy', $row->id),],
        ]);
