...
class ProjectPaymentTypeDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\ProjectPaymentType $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(ProjectPaymentType $model)
    {
        return $model->select();
    }
    ...
    public function applyFilter($query)
    {
        if(request()->order[0]['column'] != null){
            ...
            if(Schema::hasColumn('project_payment_type',$column_name)){
    ...
    {
        $options = collect([
            ['btn_file_name' => 'edit', 'permission' => 'project-payment.edit', 'link' => route('project-payment.edit', $row->id),],
            // ['btn_file_name' => 'delete', 'permission' => 'project-payment.destroy', 'link' => route('project-payment.destroy', $row->id),],
        ]);
