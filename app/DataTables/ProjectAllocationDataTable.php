...
class ProjectAllocationDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\ProjectAllocation $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(ProjectAllocation $model)
    {
        return $model->select();
    }
     ...
     public function applyFilter($query)
     {
        if(request()->order[0]['column'] != null){
            ...
            if(Schema::hasColumn('project_allocation',$column_name)){
    ...
    {
        $options = collect([
            ['btn_file_name' => 'edit', 'permission' => 'allocation.edit', 'link' => route('allocation.edit', $row->id),],
            // ['btn_file_name' => 'delete', 'permission' => 'allocation.destroy', 'link' => route('allocation.destroy', $row->id),],
        ]);
