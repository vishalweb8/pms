...
// Handle Fire Container - User cookie/db
setTimeout(function () {
	$('.user-container .user').fadeIn('500')
}, 4000);
setTimeout(function () {
	$('.user-container .user').fadeOut('500')
}, 9500);
setTimeout(function () {
	let pauseFire = store.state.paused = true;
	if (pauseFire) {
		setTimeout(function () {
			$('.fire-work-container').fadeIn('fast');
			$('.welcome_star_wrapper').fadeOut('100', function () {
				setTimeout(function () {
					$('.welcome_star_wrapper').remove();
				}, 500)
			})
		}, 100)
	}
